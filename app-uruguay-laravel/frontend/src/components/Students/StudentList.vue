...
import { getStudentsPaginated, deleteStudent, deleteStudentMultiple } from '@/services/student/StudentService';
...
const service = {
    ...
    deleteMultiple: deleteStudentMultiple
};
...
const columns = ref([
    ...
    { key: 'nationality', label: 'Nacionalidad', sortable: true, visible: true },
    { key: 'age', label: 'Edad', sortable: true, visible: true }
]);
...
<template>
    <div>
        <GenericTable :fetchDataFunction="fetchData" :columns="columns" :refreshKey="refreshKey" titleTable="Listado de estudiantes" :selectableRows="true" v-model:selectedRows="selected" @delete-selected="handleDeleteSelected">
            <template #actions>
                <Button label="Nuevo" as="router-link" :to="{ name: 'create_student' }" icon="pi pi-plus" severity="info" raised />
            </template>

            <template #row-actions="{ row }">
                <Button icon="pi pi-search" as="router-link" title="Ver detalles" :to="{ name: 'show_student', params: { id: row.id } }" outlined rounded class="mr-2" raised severity="info" />
                <Button icon="pi pi-pencil" as="router-link" title="Editar" :to="{ name: 'edit_student', params: { id: row.id } }" outlined rounded class="mr-2" raised />
