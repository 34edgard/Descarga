...
import { getSchoolYearsPaginated, deleteSchoolYear, deleteSchoolYearMultiple } from '@/services/school_year/SchoolYearService';
...
const service = {
    ...
    deleteMultiple: deleteSchoolYearMultiple
};
...
const columns = ref([
    ...
    { key: 'is_current', label: '¿Actual?', sortable: true, visible: true },
    { key: 'start_date', label: 'Fecha de Inicio', sortable: true, visible: true },
    { key: 'end_date', label: 'Fecha de Fin', sortable: true, visible: true }
]);
...
<template>
    <div>
        <GenericTable :fetchDataFunction="fetchData" :columns="columns" :refreshKey="refreshKey" titleTable="Listado de Periodos" :selectableRows="true" v-model:selectedRows="selected" @delete-selected="handleDeleteSelected">
            <template #actions>
                <Button label="Nuevo" as="router-link" :to="{ name: 'create_school_year' }" icon="pi pi-plus" severity="info" raised />
            </template>

            <template #row-actions="{ row }">
                <Button icon="pi pi-search" as="router-link" title="Ver detalles" :to="{ name: 'show_school_year', params: { id: row.id } }" outlined rounded class="mr-2" raised severity="info" />
                <Button icon="pi pi-pencil" as="router-link" title="Editar" :to="{ name: 'edit_school_year', params: { id: row.id } }" outlined rounded class="mr-2" raised />
