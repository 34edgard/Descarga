...
import { getClassroomsPaginated, deleteClassroom, deleteClassroomMultiple } from '@/services/classroom/ClassroomService';
...
const service = {
    ...
    deleteMultiple: deleteClassroomMultiple
};
...
const columns = ref([
    ...
    { key: 'section', label: 'Sección', sortable: true, visible: true },
    { key: 'capacity', label: 'Capacidad', sortable: true, visible: true }
]);
...
<template>
    <div>
        <GenericTable :fetchDataFunction="fetchData" :columns="columns" :refreshKey="refreshKey" titleTable="Listado de Salones de clases" :selectableRows="true" v-model:selectedRows="selected" @delete-selected="handleDeleteSelected">
            <template #actions>
                <Button label="Nuevo" as="router-link" :to="{ name: 'create_classroom' }" icon="pi pi-plus" severity="info" raised />
            </template>

            <template #row-actions="{ row }">
                <Button icon="pi pi-search" as="router-link" title="Ver detalles" :to="{ name: 'show_classroom', params: { id: row.id } }" outlined rounded class="mr-2" raised severity="info" />
                <Button icon="pi pi-pencil" as="router-link" title="Editar" :to="{ name: 'edit_classroom', params: { id: row.id } }" outlined rounded class="mr-2" raised />
