...
import { getSectionsPaginated, deleteSection, deleteSectionMultiple } from '@/services/section/SectionService';
...
const service = {
    ...
    deleteMultiple: deleteSectionMultiple
};
...
const columns = ref([
    ...
    { key: 'sections', label: 'Nivel', sortable: false, visible: true }
]);
...
<template>
    <div>
        <GenericTable :fetchDataFunction="fetchData" :columns="columns" :refreshKey="refreshKey" titleTable="Listado de Secciones" :selectableRows="true" v-model:selectedRows="selected" @delete-selected="handleDeleteSelected">
            <template #actions>
                <Button label="Nuevo" as="router-link" :to="{ name: 'create_section' }" icon="pi pi-plus" severity="info" raised />
            </template>

            <template #row-actions="{ row }">
                <Button icon="pi pi-search" as="router-link" title="Ver detalles" :to="{ name: 'show_section', params: { id: row.id } }" outlined rounded class="mr-2" raised severity="info" />
                <Button icon="pi pi-pencil" as="router-link" title="Editar" :to="{ name: 'edit_section', params: { id: row.id } }" outlined rounded class="mr-2" raised />
