...
import { getEnrollmentsPaginated, deleteEnrollment, deleteEnrollmentMultiple } from '@/services/enrollment/EnrollmentService';
...
const service = {
    ...
    deleteMultiple: deleteEnrollmentMultiple
};
...
<template>
    <div>
        <GenericTable :fetchDataFunction="fetchData" :columns="columns" :refreshKey="refreshKey" titleTable="Gestión de Inscripciones" :selectableRows="true" v-model:selectedRows="selected" @delete-selected="handleDeleteSelected">
            <template #actions>
                <Button label="Nueva Inscripción" as="router-link" :to="{ name: 'create_enrollment' }" icon="pi pi-plus" severity="info" raised />
                <Button label="Nuevo Estudiante" as="router-link" :to="{ name: 'create_student' }" icon="pi pi-plus" severity="info" raised />
            </template>

            <template #row-actions="{ row }">
                <Button icon="pi pi-search" as="router-link" title="Ver detalles" :to="{ name: 'show_enrollment', params: { id: row.id } }" outlined rounded class="mr-2" raised severity="info" />
                <Button icon="pi pi-pencil" as="router-link" title="Editar" :to="{ name: 'edit_enrollment', params: { id: row.id } }" outlined rounded class="mr-2" raised />
